...
class QueenOrchestrator:
    ...
    def spawn_bee(self, bee_type: str, task: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.bee_failures.get(bee_type, 0) >= self.MAX_BEE_FAILURES:
            self.log(
                f"Spawn blocked: {bee_type} is currently exiled due to failure rate.", level="error"
            )
    ...
    def trigger_event(self, event_type: str, data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for bee_type in bees_to_wake:
            task = {"triggered_by": event_type, "payload": data}
    ...
    def run_schedule(self) -> dict[str, Any]:
        ...
        for bee_type, schedule in schedules.items():
            ...
            if should_run:
                ...
                bees_run.append({"bee_type": bee_type, "result": result})
                last_runs[bee_type] = now.isoformat()

        # Update last run times
        self._update_state(
            {"scheduler": {"last_runs": last_runs, "last_schedule_check": now.isoformat()}}
        )

        return {"checked_at": now.isoformat(), "bees_run": len(bees_run), "results": bees_run}
    ...
    def heartbeat(self) -> dict[str, Any]:
        ...
        # Update state
        self._update_state({"queen": {"last_heartbeat": now.isoformat(), "status": "alive"}})
    ...
    def _read_json_cached(self, filename: str) -> dict[str, Any]:
        ...
        try:
            ...
            self._state_cache[path_str] = {"mtime": mtime, "content": content}
